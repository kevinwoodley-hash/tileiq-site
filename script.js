...
async function loadUserData() {
    ...
    // Clear local jobs — Supabase is source of truth (repopulated below)
    jobs = [];
    localStorage.setItem(LOCAL_JOBS_KEY(currentUser?.id), JSON.stringify([]));
    ...
    if (cacheAge < 5 * 60 * 1000 && jobs.length > 0) { // 5 min cache
        return; // Use cache
    }
    ...
    try {
        ...
        if (d1Resp.ok) {
            ...
            if (d1Data.jobs?.length) {
                ...
                jobs = merged;
                localStorage.setItem(LOCAL_JOBS_KEY(currentUser?.id), JSON.stringify(jobs));
                renderDashboard(); // Re-render after Supabase sync to clear stale jobs
                renderDashboard(); // Re-render after Supabase sync to clear stale jobs
                localStorage.setItem("tileiq-last-sync", Date.now().toString());
                loadedFromD1 = true;
                ...
                try {
                    ...
                    const sbResp = await fetch(`${SB_URL}/rest/v1/jobs?user_id=eq.${currentUser.id}&select=data,id,source&order=updated_at.desc`, {
        ...
        try {
            ...
            if (Array.isArray(sbJobs)) {
                ...
                for (const lj of localParsed) {
                    const idx = merged.findIndex(j => j.id === lj.id);
                    if (idx === -1) { const localAge = Date.now() - new Date(lj.createdAt || lj.updatedAt || 0).getTime(); if (localAge < 24 * 60 * 60 * 1000) merged.push(lj); }
                    else if ((lj.updatedAt || 0) > (merged[idx].updatedAt || 0)) merged[idx] = lj;
                }
                jobs = merged;
                localStorage.setItem(LOCAL_JOBS_KEY(currentUser?.id), JSON.stringify(jobs));
...
async function backgroundSync() {
    ...
    try {
        ...
        try {
            let sbToken = "";
            try { const s = localStorage.getItem("sb-lzwmqabxpxuuznhbpewm-auth-token"); if (s) sbToken = JSON.parse(s).access_token || ""; } catch(e) {}
            const sbResp = await fetch(`${SB_URL}/rest/v1/jobs?user_id=eq.${currentUser.id}&select=data&order=updated_at.desc`, {
                headers: { "apikey": SB_KEY, "Authorization": `Bearer ${sbToken || SB_KEY}` }
            });
            const sbRows = await sbResp.json();
            if (Array.isArray(sbRows) && sbRows.length) {
...
function rmSelectType(type) {
    ...
    // Default to porcelain for floor-only, ceramic for wall/room
...
function clearRoomInputs() {
    clearDeducts();
    const ids = [
        "rm-r-length","rm-r-width","rm-r-height","rm-r-deduct","rm-r-area-direct",
...
function buildSurfaces() {
    ...
    if (currentSurfType === "room") {
        ...
        if (cb("rm-r-inclfloor")) {
            surfaces.push({
                type:"floor", label:"Floor", length:rL, width:rW,
    ...
    if (currentSurfType === "floor") {
        ...
        return [{ type:"floor", label:"Floor", length:L||Math.sqrt(fAreaActual), width:W||Math.sqrt(fAreaActual),
...
function renderQuote() {
    ...
    const roomBreakdownRows = (j.rooms || []).map(room => {
        ...
        const inlineParts = [];
        if (adhBags   > 0) inlineParts.push(`Adhesive ${adhBags} × 20kg bag${adhBags !== 1 ? "s" : ""} (£${adhSell.toFixed(2)})`);
